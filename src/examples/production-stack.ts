/**
 * Example: Full Production Stack — The Showcase
 *
 * Demonstrates nearly every feature of L-Script in a production-ready setup:
 *   - ModelRouter with routing rules for different task types
 *   - FallbackProvider with OpenAI → Anthropic fallback
 *   - ExecutionCache with MemoryCacheBackend
 *   - CostTracker with BudgetConfig limits
 *   - MiddlewareManager with multiple hooks (logging, timing, error tracking)
 *   - Logger with ConsoleTransport and custom transport
 *   - Pipeline for multi-step processing
 *   - Output transformer for post-processing
 ...
 *   npx tsx src/examples/production-stack.ts
 */
...
function buildProductionStack(apiKey: string) {
  ...
  // Fallback chain: OpenAI → (would be Anthropic in prod, using OpenAI as placeholder)
  // In production you'd use: new AnthropicProvider({ apiKey: process.env.ANTHROPIC_API_KEY! })
  const fallbackProvider = new FallbackProvider([openai], {
    retryDelay: 1000,
  });
  ...
  const router = new ModelRouter({
    rules: [
      {
        // Rule 1: Classification/extraction → cheap/fast model
        match: /extract|classify/i,
        provider: openai,
        modelOverride: "gpt-4o-mini",
      },
      {
        // Rule 2: Review/analysis → frontier model
        match: /review|analyze/i,
        provider: openai,
        modelOverride: "gpt-4o",
      },
      {
        // Rule 3: Draft/generate → standard model via fallback
...
async function main() {
  ...
  if (!apiKey) {
    ...
    console.error("║  npx tsx src/examples/production-stack.ts         ║");
    console.error("╚═══════════════════════════════════════════════════╝");
    process.exit(1);
  }

  console.log("\n🏗️  Production Stack Demo");
  ...
  // Note: For pipeline to work, the output of step N must serve as input to step N+1
  // ClassifyIntent outputs IntentClassification, but GenerateDraft expects string
  // So we use a wrapper function that takes the classification JSON as input
